...
class TestConstants:
    ...
    def test_phase_values(self):
        ...
        assert Phase.DRAW == 1
        assert Phase.NOMINATE == 2
        assert Phase.PLACE == 3
...
class TestLegalActions:
    ...
    def test_draw_phase_legal_actions(self):
        """In draw phase, mayor should have 3 reveal actions."""
        game = CollapsizationGame()
        state = game.new_initial_state()
        ...
        # Should have reveal actions (0, 1, 2)
        assert len(legal) > 0
        assert all(ACTION_REVEAL_BASE <= a < ACTION_REVEAL_BASE + 3 for a in legal)
...
class TestMaxNominations:
    ...
    def test_action_build_count(self):
        ...
        # 3 hand cards × 4 max nominations = 12
        assert ACTION_BUILD_COUNT == 3 * MAX_NOMINATIONS
