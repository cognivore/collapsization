...
func test_mayor_draw_phase_shows_reveal() -> void:
	var state := _panel.compute_state(MAYOR, DRAW, -1, INVALID_HEX, [])
...
func test_mayor_draw_reveal_enabled_with_card() -> void:
	var state := _panel.compute_state(MAYOR, DRAW, 0, INVALID_HEX, [])
...
## Test: Mayor in DRAW after 2 reveals - REVEAL disabled
func test_mayor_draw_reveal_disabled_after_reveal() -> void:
	var state := _panel.compute_state(MAYOR, DRAW, 0, INVALID_HEX, [0, 1])
	assert_true(state.reveal_disabled, "REVEAL disabled after 2 cards already revealed")


## Test: Mayor in PLACE phase sees BUILD button
func test_mayor_place_phase_shows_build() -> void:
	var state := _panel.compute_state(MAYOR, PLACE, 0, VALID_HEX, [0, 1])
...
func test_mayor_place_build_disabled_without_hex() -> void:
	var state := _panel.compute_state(MAYOR, PLACE, 0, INVALID_HEX, [0, 1])
...
func test_mayor_place_build_disabled_without_card() -> void:
	var state := _panel.compute_state(MAYOR, PLACE, -1, VALID_HEX, [0, 1])
...
func test_advisor_nominate_no_commit_without_hex() -> void:
	var state := _panel.compute_state(URBANIST, NOMINATE, 0, INVALID_HEX, [0, 1])
...
func test_advisor_draw_all_hidden() -> void:
	var state := _panel.compute_state(INDUSTRY, DRAW, 0, VALID_HEX, [0, 1])
...
func test_advisor_place_all_hidden() -> void:
	var state := _panel.compute_state(URBANIST, PLACE, 0, VALID_HEX, [0, 1])
...
func test_action_card_visibility_mayor() -> void:
	var draw_state := _panel.compute_state(MAYOR, DRAW, 0, INVALID_HEX, [])
	assert_true(draw_state.show_action_card, "Action card shown for Mayor in DRAW")

	var place_state := _panel.compute_state(MAYOR, PLACE, 0, VALID_HEX, [0, 1])
